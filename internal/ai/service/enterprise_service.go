...
func (s *EnterpriseAIService) Chat(ctx context.Context, req *ChatRequest) (*ChatResponse, error) {
	...
	if req.ResponseFormat != "" {
		chatReq.ResponseFormat = &openai.ResponseFormat{Type: req.ResponseFormat}
	}
	if req.User != "" {
...
func (s *EnterpriseAIService) CreateEmbedding(ctx context.Context, req *EmbeddingRequest) (*EmbeddingResponse, error) {
	startTime := time.Now()
	atomic.AddInt64(&s.requestCount, 1)
	...
	if embReq.Model == "" {
		embReq.Model = "text-embedding-ada-002"
	}

	// 调用 API
	resp, err := s.openaiClient.CreateEmbedding(ctx, embReq)
	if err != nil {
		atomic.AddInt64(&s.failureCount, 1)
		return nil, err
	}
...
type ServiceStats struct {
	...
	ClientStats    openai.ClientStats `json:"client_stats"`
}
